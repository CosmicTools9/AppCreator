...
export function defineModuleViteConfig(options: {
  ...
  const { rootDir, port = 3000, proxyTarget = "http://localhost:8080" } = options;
  const projectRoot = path.resolve(rootDir, "../../..");

  // 映射 @alioth/* workspace 包到源码目录（使 vite dev 能直接解析）
  const frameworkAliases: Record<string, string> = {
    "@alioth/components": path.resolve(
      projectRoot,
      "Framework/frontend/components/src/index.ts",
    ),
    "@alioth/hooks": path.resolve(
      projectRoot,
      "Framework/frontend/hooks/src/index.ts",
    ),
    "@alioth/utils": path.resolve(
      projectRoot,
      "Framework/frontend/utils/src/index.ts",
    ),
    "@alioth/types": path.resolve(
      projectRoot,
      "Framework/frontend/types/src/index.ts",
    ),
    "@alioth/api": path.resolve(
      projectRoot,
      "Framework/frontend/api/src/index.ts",
    ),
    "@alioth/i18n": path.resolve(
      projectRoot,
      "Framework/frontend/i18n/src/index.ts",
    ),
    "@alioth/ontology": path.resolve(
      projectRoot,
      "Framework/frontend/ontology/src/index.ts",
    ),
    "@alioth/test-utils": path.resolve(
      projectRoot,
      "Framework/frontend/test-utils/src/index.ts",
    ),
  };
  ...
  aliasArray.push(
    { find: "@alioth/components/theme-base.css", replacement: path.resolve(componentsCssPath, "theme-base.css") },
    { find: "@alioth/components/styles.css", replacement: path.resolve(componentsCssPath, "styles.css") },
    { find: "@alioth/components/components.css", replacement: path.resolve(componentsCssPath, "styles.css") },
  );
  ...
  aliasArray.push({
    ...
    replacement: path.resolve(
      projectRoot,
      "Framework/frontend/components/src/theme-base.css",
    ),
  });
  ...
  const sortedAliases = Object.entries(frameworkAliases).sort(
  ...
  return defineConfig(({ mode }) => ({
    plugins: [react(), esmExternalRequirePlugin()],
    resolve: {
      alias: aliasArray,
    },
    server: {
...
export function defineModuleVitestConfig(options: {
  ...
  const frameworkAliases: Record<string, string> = {
    ...
    "@alioth/test-utils/vitest-setup": path.resolve(
      projectRoot,
      "Framework/frontend/test-utils/src/vitest-setup.ts",
    ),
  ...
  return defineConfig({
    ...
    // test 字段由 vitest 在 .d.ts 中扩展，Vite 的 UserConfig 不直接识别
    test: {
      environment: "jsdom",
      globals: true,
      setupFiles: [
