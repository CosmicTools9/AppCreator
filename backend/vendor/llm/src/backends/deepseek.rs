...
impl DeepSeekBackend {
    ...
    fn build_body<'a>(&'a self, req: &'a CompletionRequest) -> DeepSeekRequest<'a> {
        ...
        // DeepSeek accepts: low | medium | high. Map None/xhigh → high; minimal → low.
        let reasoning_effort: Option<&'static str> = match req.reasoning_effort {
...
impl LlmBackend for DeepSeekBackend {
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<CompletionResponse, BackendError> {
        ...
        // DeepSeek v4 may include reasoning content; preserve it as a prefix
        // so the agent can see the chain-of-thought.
