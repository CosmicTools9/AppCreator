...
use ontology_mapping::output::{
    Coordinates, EntityMapping, FieldMapping, MappingOutput, OutputMeta, Tier, TierSummary,
    TieredValue,
};
...
pub fn mapped_entities_to_mapping_output(
    ...
    let mapped: Vec<_> = entities
        .iter()
        .map(|e| ontology_mapping::output::MappedEntity {
    ...
    let summary = TierSummary {
        safe: mapped
            .iter()
            .filter(|e| e.mapping.tier == Tier::Safe)
            .count(),
        suggest: mapped
            .iter()
            .filter(|e| e.mapping.tier == Tier::Suggest)
            .count(),
        unclear: mapped
            .iter()
            .filter(|e| e.mapping.tier == Tier::Unclear)
            .count(),
    };
    MappingOutput {
        meta: OutputMeta {
