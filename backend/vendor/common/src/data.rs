...
impl<T> ApiResponse<T> {
    ...
    /// 构造失败响应（已废弃 — 请使用 AliothError 返回 HTTP 错误状态码）
    #[deprecated(
        since = "0.1.0",
        note = "Use AliothError (ApiError) to return proper HTTP error status codes instead of wrapping errors in ApiResponse with success=false"
    )]
...
mod tests {
    ...
    #[test]
    #[allow(deprecated)]
    fn test_std_api_response_error() {
        let response = ApiResponse::error(json!({"message": "failed"}));
        let parsed = serde_json::to_value(&response).unwrap();
        assert_eq!(parsed["success"], false);
        assert_eq!(parsed["data"]["message"], "failed");
    }
    ...
    fn test_api_response_error_structure() {
        let response = ApiResponse::error("some error");
        let json = serde_json::to_value(&response).unwrap();
        assert_eq!(
            json,
            serde_json::json!({"success": false, "data": "some error"})
        );
    }
